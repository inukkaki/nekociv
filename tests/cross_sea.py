...
if __name__ == "__main__":
    ...
    target = np.array([200, 0], dtype=np.float64)
    ...
    while running:
        ...
        for neighbor in ship.neighborhood + [ship]:
            x_dist_1, y_dist = target - neighbor.pos
        ...
        if ship_next == ship or np.linalg.norm(target - ship_next.pos) < 0.0:
            while True:
                target = np.array([random.random() for _ in range(2)]) - 0.5
                target_norm = np.linalg.norm(target)
                if target_norm > 0.0:
                    break
            target *= random.uniform(1.0, 10.0)/target_norm
            target += ship_next.pos
        ship = ship_next

        # Render the surface
        window.fill([0, 0, 0])
        render_rect(window, ship.row, ship.col, [255, 255, 255])
        pygame.draw.circle(window, [255, 0, 0], 2*target, 2.0)
